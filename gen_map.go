...
func (e *ExprIndex) emitMapSet(isWidth24 bool) {
	...
	emit("IMUL_NUMBER %d", unitSize) // distance from head to tail
	emit("PUSH_8")
	emit("SUM_FROM_STACK")
	emit("PUSH_8")
...
func (em *IrStmtRangeMap) emit() {
	...
	emit("%s: # begin loop ", gostring(em.labels.labelBegin))

	em.condition.emit()
	emit("TEST_IT")
	emit("je %s  # if false, exit loop", gostring(em.labels.labelEndLoop))
	...
	em.block.emit()
	emit("%s: # end block", gostring(em.labels.labelEndBlock))

	em.indexIncr.emit()

	emit("jmp %s", gostring(em.labels.labelBegin))
	emit("%s: # end loop", gostring(em.labels.labelEndLoop))
}
...
func (lit *ExprMapLiteral) emit() {
	...
	for i, element := range lit.elements {
		...
		emit("mov %%rax, %d(%%rbx) #", offsetKey) // save key address
