...
func walkExpr(expr Expr) Expr {
	...
	case *ExprFuncallOrConversion:
		...
		case builtinPanic:
			...
			staticCall.symbol = getFuncSymbol("/iruntime", identifier("iruntime"), "panic")
		...
		case builtinSyscall:
			var staticCall *IrStaticCall = &IrStaticCall{
				...
				origExpr: funcall,
				callee:   decl,
				symbol:   getFuncSymbol("/iruntime", identifier("iruntime"), "syscall"),
				args:     funcall.args,
			}
			return staticCall
		case builtinMakeSlice:
			...
			staticCall.symbol = getFuncSymbol("/iruntime", identifier("iruntime"), "makeSlice")
			staticCall.args = funcall.args
			return staticCall
		case builtinAppend:
			...
			case 1:
				staticCall.symbol = getFuncSymbol("/iruntime", identifier("iruntime"), "append1")
			case 8:
				staticCall.symbol = getFuncSymbol("/iruntime", identifier("iruntime"), "append8")
			case 24:
				...
				staticCall.symbol = getFuncSymbol("/iruntime", identifier("iruntime"), "append24")
			default:
				TBI(slice.token(), "")
			}
			staticCall.args = funcall.args
