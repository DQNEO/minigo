...
func doEmitData(ptok *Token /* left type */, gtype *Gtype, value /* nullable */ Expr, containerName string, depth int) {
	...
	if primType == G_ARRAY {
				...
				if size == 8 {
					if value.getGtype().getKind() == G_STRING {
						...
						case *ExprUop:
							...
							emit(".quad %s # %s %s", gostring(vr.varname), gostring(value.getGtype().String()), gostring(selector))
						case *ExprVariable:
		...
		if value == nil {
			// zero value
			emit(".quad 0 # %s %s",  gostring(gtype.String()), gostring(containerName))
			return
		}
		var val int = evalIntExpr(value)
		emit(".quad %d # %s %s", val, gostring(gtype.String()), gostring(containerName))
		...
		switch value.(type) {
		case nil:
			emit(".quad %d # %s %s zero value", val, gostring(gtype.String()), gostring(containerName))
		...
		case *ExprConstVariable:
			...
			emit(".quad %d # %s ", val, gostring(gtype.String()))
		case *ExprVariable:
